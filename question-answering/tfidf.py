...
class TfIdfModel:
    ...
    def train(self, sentences):
        ...
        for i in range(len(sentences)):
            ...
            self.tfidf.add_document(i, sentence.split())

    def getNRelevantSentences(self, sentence, n):
        sentence = sentence.translate(None, string.punctuation)
        similarities = self.tfidf.similarities(sentence.split())
        similarities.sort(key=lambda x: x[1], reverse=True)
        similarities = similarities[0:n]
        return [(self.sentences[x[0]],x[1]) for x in similarities]
